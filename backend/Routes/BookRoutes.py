...
async def upload_book(
    ...
    try:
        book_dict = await book_service.process_uploaded_book(
            file_path, file.filename, user_id
        )
        return JSONResponse(
            content={
                "status": "success",
...
async def update_book_progress(
    ...
    totalPages: int = Query(
        ..., description="Общее количество страниц текущего текста"
    ),
):
    try:
        logger.info(
            f"Запрос на обновление прогресса: bookId={bookId}, blockStopBook={blockStopBook}, totalPages={totalPages}"
        )

        # Обновляем прогресс через репозиторий
        update_result = (
            await book_service.book_repository.update_book_progress_by_block(
                bookId, blockStopBook, totalPages
            )
        )
        ...
        raise HTTPException(
            status_code=500, detail=f"Ошибка обновления прогресса: {str(e)}"
        )
...
async def get_book_detail(bookId: str = Query(..., description="ID книги")):
    """
    ...
    try:
        ...
        for block_id in text_block_ids:
            block_doc = await book_service.book_repository.get_text_block_by_id(
                block_id
            )
            if block_doc:
        ...
        for compression_key, block_ids in compressed_text.items():
            ...
            compressed_docs = (
                await book_service.book_repository.get_compressed_blocks_by_ids(
                    block_ids
                )
            )
...
@book_router.put(
    "/api/book/updateCompressionLevel",
    summary="Обновить уровень сжатия книги",
    description="",
    tags=["Загрузка и работа с файлами"],
)
async def update_compression_level(
    ...
    try:
        logging.info(
            f"Received request to update compression level for bookId={bookId}, compressionLevel={request.compressionLevel}"
        )

        # Проверяем границы значений
        if not (0 <= request.compressionLevel <= 75):
            raise HTTPException(
                status_code=400, detail="Уровень сжатия должен быть от 0 до 75"
            )

        # Обновляем уровень сжатия через репозиторий
        result = await book_service.book_repository.update_book_field(
            bookId, "compressionLevel", request.compressionLevel
        )
        if result.matched_count == 0:
            raise HTTPException(
                status_code=404, detail="Книга не найдена или не удалось обновить"
            )
        ...
        return JSONResponse(
            content={"status": "success", "message": "Уровень сжатия успешно обновлен"}
        )
    except Exception as e:
        logging.error(f"Error updating compression level for bookId={bookId}: {e}")
        raise HTTPException(
            status_code=500, detail=f"Ошибка обновления уровня сжатия: {str(e)}"
        )


@book_router.post(
    "/api/book/{book_id}/generate-tests",
    summary="Генерация тестов для книги",
    tags=["Тесты"],
)
async def generate_tests(
    book_id: str,
    test_type: str = Query(..., description="Тип теста (key_points, main_ideas)"),
    stop_block: int = Query(..., description="Номер последнего прочитанного блока"),
):
    try:
        tests = await book_service.generate_tests(book_id, test_type, stop_block)
        return JSONResponse(content={"status": "success", "tests": tests})
    except HTTPException as e:
        raise e
    except Exception as e:
        raise HTTPException(
            status_code=500, detail=f"Ошибка генерации тестов: {str(e)}"
        )
