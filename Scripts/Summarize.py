...
async def process_chunk(book_reader, arr_block, process_queue, chapters, mode):
    while True:
...
async def main(name_file, need_questions, questions, answer_user, book_reader, mode):
    ...
    task_chunk = asyncio.create_task(process_chunk(book_reader, arr_block, process_queue, chapters, mode))
...
def control(name_file, questions, answer, book_reader):
    ...
    while mode != "time" and mode != "sum":
        mode = input("Read in time or randomly summarized text(time/sum): ")
    if mode == "time":
        asyncio.run(main(name_file, need_questions, questions, answer, book_reader, "time"))
    if mode == "sum":
        asyncio.run(main(name_file, need_questions, questions, answer, book_reader, "sum"))
